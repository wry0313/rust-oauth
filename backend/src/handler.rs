...
async fn register_user_handler(
    mut body: web::Json<RegisterUserSchema>,
    data: web::Data<AppState>,
) -> impl Responder {
    let mut vec = data.db.lock().unwrap();
    ...
    let user = User {
        ...
        name: std::mem::take(&mut body.name),
        verified: false,
        email: std::mem::take(&mut body.email).to_lowercase(),
...
async fn login_user_handler(
    ...
    let jwt_secret = data.env.jwt_secret.as_ref();
    let now = Utc::now();
    let iat = now.timestamp() as usize;
    let exp = (now + Duration::minutes(data.env.jwt_max_age)).timestamp() as usize;
